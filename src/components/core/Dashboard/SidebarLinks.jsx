...
const SidebarLinks = ({ element, currentClicked, setCurrentClicked }) => {
  ...
  return (
    <Link to={element.path}>
      <div onClick={() => setCurrentClicked(element.name)} className={` flex flex-row gap-3 py-2 px-6 items-center ${currentClicked === element.name ? "bg-yellow-800 text-yellow-50 border-l-2 border-yellow-50 " : ""} `} >
